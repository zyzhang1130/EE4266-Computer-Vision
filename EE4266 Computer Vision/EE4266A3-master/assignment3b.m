...
freq3=[freq3(2:keep+1);freq3(end-keep+1:end)];
...
freq=[freq(2:keep+1);freq(end-keep+1:end)];
freq=freq/norm(freq);
freq=abs(freq);
%meann=[meann,std(abs(freq./freq3))];
v = keep*2:-1:1;
meann=[meann,v*abs(freq-freq3)];

%if std(abs(freq./freq3))<0.64
if v*abs(freq-freq3)<2.8
